#bags = open('data/test_bags_2.txt')
...
def depth_sum(color):
    ...
    if not bags_dict[color]:
        ...
        for key in list(bags_dict[color].keys()):
            #print(key)
            #print(bags_dict[color][key])

            subtotal = (bags_dict[color][key] * depth_sum(key)) + bags_dict[color][key]
            total += subtotal
        return total

print(depth_sum(my_bag))